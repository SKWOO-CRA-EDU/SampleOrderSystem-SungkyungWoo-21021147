...
## 1.6 축3 재검토 — 누락 파일 4건과 ADR-I3 범위 정정

축3 최초 집계("이식 12 / 잔류 0 / 보류 12")는 **행(row) 개수**였다 — 표의 한 행이 여러
파일을 묶는 경우가 있어 실제 **파일 개수**와 다르다. 4개 PoC의 실제 파일 총계
(main.cpp 4종 제외, `find`로 재확인)는 ConsoleMVC 23 / DataPersistence 11 /
DataMonitor 12 / DummyDataGenerator 7 = **53개**다. 기존 표(§4)가 실제로 다루던
파일은 49개였고, 다음 4개가 어느 절에도 등장하지 않았다(판정 없음):

- `src/infra/console/ConsoleInputAdapter.{h,cpp}` (ConsoleMVC) — `IInputPort`의 실제
  구현(`std::cin`에서 한 줄을 읽음). 최초 패턴 검색이 `*OutputAdapter*` 계열만 훑어
  놓쳤다. 이미 "이식(재작성 대상)" 판정을 받은 ConsoleOutputAdapter/PlainConsoleOutputAdapter와
  구조·계약 정합성이 동일하다(같은 폴더, IInputPort 시그니처 그대로 구현) — §4에 이식으로 추가한다.
- `JsonValue.{h,cpp}` (DataMonitor) — 계약에 없는 내부 JSON 파서(DataPersistence의
  MiniJson과 같은 성격). `SnapshotLoader.cpp`에서만 사용되며, SnapshotLoader 자체가
  이미 "보류"(Domain.h 미확정에 종속) 판정이므로 JsonValue도 그 판정에 종속된다 —
  §4에 보류로 추가한다.

**판정 없는 파일 0건**은 이 두 쌍(4개 파일)을 §4 표에 추가한 뒤에야 충족된다(아래 반영).

**ADR-I3와 "잔류" 표현에 대하여**: 지시문은 "ADR-I3가 DataMonitor의 통합 전체 제외를
결정했으므로 그 파일들은 전량 잔류여야 한다"는 전제였다. 그러나 DECISIONS.md I3 결정문
원문은 다음과 같다 — "결정: DataMonitor의 **프로세스 아키텍처**(별도 프로세스가 동일
저장 파일을 동시에 읽는 구조)는 본 프로젝트에 이식하지 않는다", "**'통합 제외'는 'repo
폐기'가 아니다**... **제외되는 것은 프로세스 아키텍처이지 기능이 아니다**. FR-21/FR-22는
살아 있으며... 단일 프로세스 내 Model 계산 + View 표시로 제공된다." 즉 I3가 제외한 것은
DataMonitor의 파일 전체가 아니라 "별도 프로세스가 파일을 동시에 읽는다"는 설계 하나이고,
기능(FR-21/22)은 이 프로젝트 안에서 재작성되어 존속한다. 이는 ConsoleMVC/DataPersistence가
"이식(재작성 대상)"으로 판정된 것과 같은 축 위에 있다 — 다만 DataMonitor는 계약 자체가
미갱신(v1)이라 무엇이 재작성 형태로 남을지 이번 조사만으로 확정할 수 없어 "보류"다.
따라서 DataMonitor 파일 전체를 "잔류"(변경 없이 그대로 제외, 아무것도 남지 않음)로
못박는 것은 I3가 실제로 결정한 범위(프로세스 아키텍처 하나)보다 넓게 해석하는 것이 되어
**I3를 재해석하는 셈**이 된다. 지시문 (1)의 전제와 어긋나므로, I3의 문언 그대로
"기능은 존속·구조는 미확정"으로 남기고, DataMonitor 파일들은 지금처럼 "보류"를 유지한다.
이 원장은 애초에 "잔류"라는 판정값을 어느 행에도 부여한 적이 없다 — §4 집계 줄의
"잔류 0건"은 그래서 나온 숫자이지, 계산 누락이 아니다. main.cpp 4종은 §4-(d)에서
"판정하지 않음"으로 별도 추적되며, 이식/보류 집계(위 53개)에는 포함되지 않는다.
...
**구조**: `src/app`(Controller), `src/infra/console`(View: ConsoleOutputAdapter/PlainConsoleOutputAdapter 2종, Input: ConsoleInputAdapter), `src/infra/repository`(InMemorySampleRepository/InMemoryOrderRepository), `src/ports`(IInputPort/IOutputPort/ISampleRepository/IOrderRepository), `tests`(ControllerScenarioTests, fakes).
...
**구조**: `Domain.h`(계약 상당), `Snapshot.h`(RootSnapshot 값 객체), `SnapshotLoader.{h,cpp}`(파일 I/O·파싱 전담, Repository 아님 — `ReadOutcome{Success,NotFound,ParseError,UnsupportedSchema}` 자체 어휘 사용), `JsonValue.{h,cpp}`(SnapshotLoader 전용 내부 JSON 파서, 계약에 없음 — DataPersistence의 MiniJson과 같은 성격), `FileSnapshotRepository.{h,cpp}`, `InMemoryRepository.{h,cpp}`, `ConsoleView.{h,cpp}`(FR-04, ISample/IOrderRepository만 의존).
...
**불일치 건수: 6/6.**[^1] DataPersistence만 6개 전부 접촉(실제 파일 저장소 구현체를 갖고
...
[^1]: 이 숫자는 "4개 PoC 중 몇 개가 이 어휘를 접촉했는가"를 어휘별로 센 것이며,
미접촉 2개(DataMonitor·DummyDataGenerator)를 분모(4)에 그대로 포함한다. 미접촉은
"계약을 읽고 다르게 해석한 것"(drift)이 아니라 "계약을 읽지 않은 것"이다 — 대조가
성립하려면 최소 두 구현이 같은 어휘를 실제로 사용해야 하므로, 아래 §3.1에서 실질
대조 대상과 미접촉을 구분한다.

### 3.1 실질 대조 vs 미접촉 구분

| 구분 | PoC | 6개 어휘 중 실제 사용 | 성격 |
|---|---|---|---|
| 실질 대조 대상 | DataPersistence | 6/6 | 계약 어휘의 기준 구현([D] 3종 + [F] 3종 전부) |
| 실질 대조 대상 | ConsoleMVC | 3/6 | 계약 어휘의 부분 구현([D] `WriteOutcome` 3종만; [F] 예외 3종은 인메모리라 발생 매체 자체가 없어 비교 불성립) |
| 미접촉 | DataMonitor | 0/6 | 자체 `ReadOutcome` 어휘 사용, 계약 어휘 비참조(§2.3) |
| 미접촉 | DummyDataGenerator | 0/6 | `Add`/`Update` `void` 반환, 계약 어휘 비참조(§2.4) |

**실질 대조 결과(DataPersistence와 ConsoleMVC가 겹치는 범주 — [D] `WriteOutcome` 3종에서)**:
`WriteOutcome::Ok/NotFound/DuplicateKey`를 Add/Update 반환값으로 동일하게 사용한다 —
**일치**. 겹치지 않는 범주([F] 예외 3종)는 ConsoleMVC가 인메모리 구현이라 애초에 발생
매체가 없어 비교 자체가 성립하지 않는다(drift 아님, §2.1). 즉 실질 대조 대상 둘 사이의
어휘 사용은 접촉한 범위 안에서 전부 일치하며, 원 표의 "6/6 불일치"가 가리키는 갈라짐은
어휘 사용의 불일치가 아니라 **PoC별 어휘 접촉 여부의 편차**다. 실제 의미론적 drift는
§1.2의 `[[nodiscard]]` 누락 1건뿐이다.
...
| `src/infra/console/ConsoleInputAdapter.{h,cpp}` | ConsoleMVC | 이식(재작성 대상) | IInputPort 구현(std::cin), §1.6 재검토에서 누락 확인 후 추가 | - |
...
| `tests/ControllerScenarioTests.{h,cpp}`, `tests/fakes/FakeInputPort.{h,cpp}` | ConsoleMVC | 이식(재작성 대상) | (a) 판정 확정, §5-(a) 참조 — RecordingOutputPort를 소비하는 시나리오 테스트 | - |
| `tests/fakes/RecordingOutputPort.{h,cpp}` | ConsoleMVC | 이식(재작성 대상) | (a) 판정 확정, §5-(a) 참조 — ADR-H5가 View 관통 관측 축의 참조 설계로 채택 | ADR-H5 |
...
| `src/OrderStatusCodec.{h,cpp}` | DataPersistence | 이식(재작성 대상) | (c) 판정 확정, §5-(c) 참조 — CONTRACT.md §3 wire format 매핑표와 완전 일치, drift 없음 | CONTRACT §3 |
| `Domain.h` | DataMonitor | 보류 | 미갱신(§1.5), v3 재작성 시 형태가 전부 바뀌어야 함 — 무엇이 남을지 이 조사만으로 확정 불가(§1.6: ADR-I3는 프로세스 아키텍처만 제외, 파일 전체 잔류를 결정하지 않음) | ADR-I3 |
...
| `JsonValue.{h,cpp}` | DataMonitor | 보류 | SnapshotLoader 전용 내부 파서 — SnapshotLoader 판정에 종속, §1.6 재검토에서 누락 확인 후 추가 | - |
...
| `DummyDataGenerator.{h,cpp}` | DummyDataGenerator | 이식(재작성 대상) | (b) 판정 확정, §5-(b) 참조 — `orderRepo.NextOrderId()` 호출부는 Model 채번으로 재작성 | ADR-E4 |
| `InMemoryRepository.{h,cpp}`(DummyDataGenerator) | DummyDataGenerator | 이식(재작성 대상) | (b) 판정 확정, §5-(b) 참조 — 나머지는 이식하되 `NextOrderId()` 메서드는 v3 재작성 시 제거 대상 | ADR-E4 |
| `Tests.{h,cpp}` | DummyDataGenerator | 보류 | FR-13 재현성 검증 스위트 — 대상 인터페이스(Contract.h) 판정 미확정 | ADR-I4(assert-count 방식과 관계) |

**건수 집계(행 기준, §5 (a)(b)(c) 판정 확정 반영 후)**: 이식(재작성 대상) 18행 / 보류 8행 = 26행
(총 행수는 §1.6 시점과 동일 — (a)(b)(c) 5행이 보류→이식으로 재분류된 것뿐, 행 자체를
추가·삭제하지 않았다).
**건수 집계(파일 기준)**: 이식 26개→31개 / 보류 27개→22개 = 53개(변동 없음, main.cpp 4종
제외 전체 파일 수와 일치, §1.6). "잔류"는 이 원장에서 어느 행에도 부여된 적 없는
판정값이다 — 0건은 계산 결과가 아니라 애초에 쓰지 않은 범주라는 뜻이다(§1.6 참조,
ADR-I3는 DataMonitor 전체가 아니라 프로세스 아키텍처만 제외했으므로 "전량 잔류"로
못박지 않는다).
main.cpp 4종의 소재는 §4-(d)에서 별도로 다룬다(충돌만 기록, 판정하지 않음 — 위 53개
집계에 포함되지 않음). 남은 보류 8행은 전부 DataMonitor(6행, Domain.h 미확정에 종속)와
DummyDataGenerator `Contract.h`/`Tests.{h,cpp}`(2행, 계약 헤더 형태 자체가 v3 재작성 시
전부 바뀌어야 해서 이 조사만으로 확정 불가 — §1.4)다.

### 보류 목록 상세 — (a)(b)(c) 3건 판정 확정, (d)는 지시에 따라 판정 보류 유지

**(a) `tests/ControllerScenarioTests.{h,cpp}`, `tests/fakes/RecordingOutputPort.{h,cpp}` — 최우선 확인 항목 [판정 확정]**
...
코드·상세)을 구조화된 값으로 기록하고, 테스트는 그 구조화된 값을 단언한다.

**판정**: 이식(재작성 대상). 이 원장이 위에서 확인한 사실 자체가 이후 DECISIONS.md
ADR-H5의 근거로 채택됐다 — ADR-H5(H 계열, View 관통 관측 축)는 정확히 이
`RecordingOutputPort` 구조("`IOutputPort` 5개 메서드 각각이 `events.push_back(...)`만
수행하며 문자열 포맷팅/렌더링 코드가 전혀 없다")를 인용해 "Controller→View 관통을
구조화된 값(Kind+인자) 기록으로 관측한다"는 3번째 관측 축의 참조 설계로 삼았다.
즉 판정 보류 상태였던 이 파일 쌍은 이미 사후적으로 이 프로젝트의 Harness 설계에
채택된 셈이다 — 화면 문자열 비교 금지(CLAUDE.md/ADR-H1/H5)를 지키는 View 테스트 더블
패턴으로 그대로 재작성 대상에 넣는다. `ControllerScenarioTests`는 `RecordingOutputPort`를
소비하는 시나리오 테스트이므로 같은 판정을 따른다.

**(b) DummyDataGenerator 채번 [판정 확정]**
...
`IOrderRepository`에 `NextOrderId()`를 인터페이스 메서드로 선언하고 있다.

**판정**: drift 확정, 재작성 시 계층 이동. DECISIONS.md ADR-E4가 `IOrderRepository::NextOrderId`를
계약에서 전면 제거하기로 이미 결정했다 — 이유는 채번 방식(자동증가/일자별 등)과 무관하게
"채번은 도메인 서비스(Model)의 책임"이며, `int64_t` 단일 반환값에는 저장 매체 접근 실패를
표현할 자리가 없어 임의값이 정상값으로 둔갑할 수 있기 때문이다(ADR-E4 근거). ADR-E4가 명시한
대체 경로 — "도메인 서비스가 `FindAll()`로 기존 최대 orderId를 구해 +1한다" — 는
`InMemoryRepository.cpp:52-56`의 계산 로직과 정확히 같다. 즉 계산 로직 자체는 버릴 것이
아니라 **계층만 옮긴다**: `NextOrderId()`를 `IOrderRepository`에서 제거하고 그 안에 있던
"FindAll 최대값+1" 계산을 Model(도메인 서비스)로 이동한다. `DummyDataGenerator.{h,cpp}`는
이식(재작성 대상)이나, `orderRepo.NextOrderId()` 호출부는 Model이 채번한 값을 받는 형태로
재작성해야 한다. `InMemoryRepository.{h,cpp}`(DummyDataGenerator)도 이식(재작성 대상)이나,
`NextOrderId()` 메서드 자체는 v3 재작성 시 제거 대상이다(ADR-E4 "이 결정이 위반되면
실패해야 할 테스트" 참고 — `IOrderRepository`에 동등 메서드가 재도입되면 실패해야 함).

**(c) OrderStatusCodec [판정 확정]**
...
**판정**: 이식(재작성 대상), drift 없음. 매핑 문자열 5쌍이 `docs/CONTRACT.md:63-67`(§3
wire format 매핑표)과 완전히 일치하고, 헤더 주석이 스스로 "계약에 없는 내부 유틸리티...
CONTRACT.md v3 §2 매핑 표를 그대로 코드로 옮긴 것"이라고 밝히고 있다. DataPersistence의
`MiniJson.{h,cpp}`("저장 방식은 저장소 자유"에 속하는 계약 밖 내부 유틸)와 같은 성격이며,
그것이 이미 이식(재작성 대상)으로 판정된 것과 같은 근거로 판정한다.

**(a)(b)(c) 판정에 따른 §4 표 갱신**: `tests/ControllerScenarioTests.{h,cpp}`/
`tests/fakes/FakeInputPort.{h,cpp}`, `tests/fakes/RecordingOutputPort.{h,cpp}`(ConsoleMVC),
`src/OrderStatusCodec.{h,cpp}`(DataPersistence), `DummyDataGenerator.{h,cpp}`,
`InMemoryRepository.{h,cpp}`(DummyDataGenerator) — 이상 5행을 "보류"에서
"이식(재작성 대상)"으로 갱신한다(§4 표 반영). 갱신 후 집계: **이식 18행 / 보류 8행 = 26행**
(행 기준, 총 행수는 불변). `Contract.h`(DummyDataGenerator)와 DataMonitor 6행(Domain.h 계열)은
이번 3건과 무관하므로 "보류"를 유지한다(각각 §1.4, §1.5/§1.6 사유).
...
이 원장의 최초본은 `86543ed`로 커밋됐고(파일 1개, `docs/BACKPORT-v3.md`), 이후 정정본은
`da24de7`(DataMonitor 판정·contact-vs-drift 구분)과 (a)(b)(c) 미결 3건 판정 확정을 반영한
본 커밋으로 이어졌다. 아래 "커밋 확인 예정"이었던 이전 문구는 커밋 완료 후에도 갱신되지
않은 채로 커밋 본문에 남아 있던 오기였다 — 여기서 정정한다.
