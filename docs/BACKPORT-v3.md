...
**건수 집계(행 기준, §1.6 반영 후)**: 이식(재작성 대상) 13행 / 보류 13행 = 26행.
**건수 집계(파일 기준)**: 이식 26개 / 보류 27개 = 53개(main.cpp 4종 제외 전체 파일 수와
일치, §1.6). "잔류"는 이 원장에서 어느 행에도 부여된 적 없는 판정값이다 — 0건은
계산 결과가 아니라 애초에 쓰지 않은 범주라는 뜻이다(§1.6 참조, ADR-I3는 DataMonitor
전체가 아니라 프로세스 아키텍처만 제외했으므로 "전량 잔류"로 못박지 않는다).
